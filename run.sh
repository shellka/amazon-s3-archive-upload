...
if [ ! -n "$WERCKER_S3_ARCHIVE_UPLOAD_BUCKET2" ]; then
  #set default bucket
  export WERCKER_S3_ARCHIVE_UPLOAD_BUCKET2="wercker-deployments"
fi

if [ ! -n "$WERCKER_S3_ARCHIVE_UPLOAD_REGION2" ]; then
...
echo "region = $WERCKER_S3_ARCHIVE_UPLOAD_REGION2" >> $HOME/.aws/config
...
export AWS_DEFAULT_REGION=$WERCKER_S3_ARCHIVE_UPLOAD_REGION2
...
aws s3 cp --acl private $AWS_APP_FILENAME s3://$WERCKER_S3_ARCHIVE_UPLOAD_BUCKET2
